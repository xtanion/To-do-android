...
////            }
    ...
    override fun onCheckboxClick(data:TodoEntity) {
        ...
        if (!data.completed){
            val newData = TodoEntity(data.id,data.title,data.description,data.important,true,data.groupName,date_time,data.nestedTodo,data.bitmap,data.requestCode,data.alarmTime,data.dueDate)
            mViewModel.updateTodo(newData)

        }
        else{
            val newData = TodoEntity(data.id,data.title,data.description,data.important,false,data.groupName,date_time,data.nestedTodo,data.bitmap,data.requestCode,data.alarmTime,data.dueDate)
            mViewModel.updateTodo(newData)
        }
    }
    ...
    override fun onStarClick(data: TodoEntity) {
        if (data.important){
            val newData = TodoEntity(data.id,data.title,data.description,false,data.completed,data.groupName,date_time,data.nestedTodo,data.bitmap,data.requestCode,data.alarmTime,data.dueDate)
            mViewModel.updateTodo(newData)

        }else{
            val newData = TodoEntity(data.id,data.title,data.description,true,data.completed,data.groupName,date_time,data.nestedTodo,data.bitmap,data.requestCode,data.alarmTime,data.dueDate)
            mViewModel.updateTodo(newData)
        }
    }
